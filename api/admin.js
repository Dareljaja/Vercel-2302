...
async function handleProducts(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const product = {
      ...
      stock: (input.stock === undefined || input.stock === null || input.stock === '') ? null : Math.max(0, parseInt(input.stock, 10) || 0),
    ...
    const forFrontend = { ...data, name: data.nombre, price: data.precio, description: data['descripcion completa'], shortDescription: data['descripcion corta'], image_url: data.imagen_url, category: data.categoria, size: data.tamaño, ingredients: data.ingredientes, howToUse: data['modo de uso'], stock: data.stock ?? null, popular: data.popular ?? false, offer: data.offer ?? false, precio_oferta: data.precio_oferta != null ? Number(data.precio_oferta) : null, showDescription: vis.showDescription !== false, showShortDescription: vis.showShortDescription !== false, showSize: vis.showSize !== false, showIngredients: vis.showIngredients !== false, showHowToUse: vis.showHowToUse !== false };
